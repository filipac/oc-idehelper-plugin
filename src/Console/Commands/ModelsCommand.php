...
namespace Filipac\IdeHelper\Console\Commands;
...
\n\n";
        ...
        foreach ($models as $name) {
            ...
            if (class_exists($name)) {
                try {
                    ...
                    if ($reflectionClass->hasProperty('extensionData') && $reflectionClass->hasMethod('addDynamicMethod')) {
                        $property = $reflectionClass->getProperty('extensionData');
                        $property->setAccessible(true);
                        $propertyAppends = $reflectionClass->getProperty('appends');
                        $propertyAppends->setAccessible(true);
                        $handleRelation = $reflectionClass->getMethod('handleRelation');
                        $handleRelation->setAccessible(true);
                        ...
                        if ($reflectionClass->isSubclassOf(DatabaseModel::class)) {
                            foreach ($reflectionClass->newInstance()->getRelationDefinitions() as $type => $defs) {
                                if (strpos($type, 'Many') !== false) {
                                    foreach ($defs as $relname => $def) {
                                        $relationObj = $handleRelation->invoke($model2, $relname);
    ...
    protected function addProps(array $methods, $model, $extensionData = null)
    {
        foreach ($methods as $method) {
            if (
                ...
                if (! empty($name)) {
                    $reflection = $this->getAReflection($model, $method, $extensionData);
                ...
                if (! empty($name)) {
                    ...
                    $builder = $this->getClassNameInDestinationFile(
                        $reflection instanceof ReflectionFunction ? $this->modelClass : $reflection->getDeclaringClass(),
                        \Illuminate\Database\Eloquent\Builder::class
                    );
                    $modelName = $this->getClassNameInDestinationFile(
                        $reflection instanceof ReflectionFunction ? $this->modelClass : $reflection->getDeclaringClass(),
                        $reflection instanceof ReflectionFunction ? get_class($this->modelClass) : $reflection->getDeclaringClass()->getName(),
                    );
                ...
                foreach (
                    ...
                    if (stripos($code, $search) || ltrim($impl, '\\') === ltrim((string) $type, '\\')) {
                        ...
                        $methodReflection = $this->getAReflection($model, $method, $extensionData);
    ...
    public function getParameters($method)
    {
        ...
        foreach ($method->getParameters() as $param) {
            ...
            $paramStr = (! is_null($paramClass) ? '\\'.$paramClass.' ' : '').'$'.$param->getName();
            $params[] = $paramStr;
    ...
    protected function getReturnType($reflection): ?string
    {
     ...
     * @return null|string
     */
    protected function getReturnTypeFromDocBlock($reflection)
    {
    ...
    protected function getReturnTypeFromReflection($reflection): ?string
    {
